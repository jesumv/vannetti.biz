...
if(!isset($_GET['oc'])){$result= 0 ;}else{
    ...
    if (is_object($mysqli)) {
    	$oc= $_GET['oc'];
    	$sqlCommand = "SELECT t1.idartsoc, t1.cant, t2.nom_corto, t2.speso, t2.costo,t2.iva FROM artsoc AS t1 LEFT JOIN productos AS t2
...
//inicializacion de arreglo
			...
			if($filas > 0){
				while($tempo=mysqli_fetch_array($query1, MYSQLI_ASSOC)){
			 	...
			 	'speso' => $tempo['speso'],'costo' => $tempo['costo'],'civa'=>$tempo['iva']);
			 }
			}else{$result= 1;}
